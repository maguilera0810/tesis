...
def texto_data(screen, pos_x=TEXT_POS_X, pos_y=TEXT_POS_Y, data_r=None, data_i=None, data_f=None, ancho=TEXT_WIDTH, alto=TEXT_HEIGHT, style=STYLE_1):
    ...
    if data_r != None:
        ...
        posy += desp
        Texto = style.render(
            f"Anomalous score = {round(data_r.get('anomalous')*100,dig)}%", True, white)
        screen.blit(Texto, (pos_x + 50, posy))
        posy += desp
        Texto = style.render(
            f"Normal Score = {round(data_r.get('normal')*100,dig)}%", True, white)
        screen.blit(Texto, (pos_x + 50, posy))
        posy += desp
    if data_i != None:
        ...
        posy += desp
        Texto = style.render(
            f"Anomalous score = {round(data_i.get('anomalous')*100,dig)}%", True, white)
        screen.blit(Texto, (pos_x + 50, posy))
        posy += desp
        Texto = style.render(
            f"Normal Score = {round(data_i.get('normal')*100,dig)}%", True, white)
        screen.blit(Texto, (pos_x + 50, posy))
        posy += desp
    if data_f != None:
        ...
        posy += desp
        Texto = style.render(
            f"Anomalous score = {round(data_f.get('anomalous')*100,dig)}%", True, white)
        screen.blit(Texto, (pos_x + 50, posy))
        posy += desp
        Texto = style.render(
            f"Normal Score = {round(data_f.get('normal')*100,dig)}%", True, white)
        screen.blit(Texto, (pos_x + 50, posy))
        posy += desp
                            ...
                            if not extract_texting.checkear(video.tramos_no_usar, fps, i):
                                ...
                                if i % 25 == 0:
                                    ...
                                    if prueba3:
                                        ...
                                        y_pred_f.append(
                                            data_frankensnet.get("anomalous"))
                                    else:

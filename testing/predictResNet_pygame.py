...
resnet = os.path.join(paths.checkpoints_resnet, "ResNet50_model_weights.h5")
inception = os.path.join(paths.checkpoints_inception,
                         "InceptionV3_model_weights.h5")
frankensnet = os.path.join(
    paths.checkpoints_frankensnet, "FrankensNet_model_weights.h5")
modelos = [resnet, inception, frankensnet]
modelo = modelos[int(sys.argv[2])]
print(modelo)
...
classifier1 = load_model(modelos[0])
classifier2 = load_model(modelos[1])

test_image = image.load_img("84985.jpg", target_size=(WIDTH, HEIGHT))
test_image = image.img_to_array(test_image)
test_image = np.expand_dims(test_image, axis=0)
print("resnet")
result1 = classifier1.predict(test_image, verbose=1)
print(result1)
data = {"anomalous": result1[0][0], "normal": result1[0][1]}
print(data)
print(data.get("anomalous"))
print("inception")
result2 = classifier2.predict(test_image, verbose=1)
print(result2)
data = {"anomalous": result1[0][0], "normal": result1[0][1]}
print(data)
print(data.get("anomalous"))
...
f_n, f_a= leertxt.leer(
    paths.normal_testing_data_txt,
    paths.anomalous_testing_data_txt
)
#print(len(f_a),vars(f_a[0]))
#print(len(f_n),vars(f_n[0]))
if False:

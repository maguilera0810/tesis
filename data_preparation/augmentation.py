...
def crop_image(image, y1, y2, x1, x2):
    ...
    cv2.imwrite(Folder_name+"/Crop-"+str(x1)+str(x2) +
                "*"+str(y1)+str(y2)+Extension, image)
...
def padding_image(image, topBorder, bottomBorder, leftBorder, rightBorder, color_of_border=[0, 0, 0]):
    ...
    cv2.imwrite(Folder_name + "/padd-" + str(topBorder) + str(bottomBorder) +
                "*" + str(leftBorder) + str(rightBorder) + Extension, image)
...
def add_light_color(image, color, gamma=1.0):
    ...
    if gamma >= 1:
        ...
        cv2.imwrite(Folder_name + "/dark_color" +
                    str(gamma) + Extension, image)
...
def saturation_image(image, saturation):
    ...
    cv2.imwrite(Folder_name + "/saturation-" +
                str(saturation) + Extension, image)
